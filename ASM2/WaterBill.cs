...
namespace ASM2
{
    public partial class WaterBill : Form
    {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please enter your Fullname", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            string customerType = cobCustomerType.Text.Trim().ToLower();
            if (string.IsNullOrEmpty(customerType))
            {
                MessageBox.Show("Please select Customer's Type", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            int countMember = 0;
            if (customerType.Equals("household customer"))
            {
                ...
                {
                    MessageBox.Show("Enter number of family member, please try again", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                countMember = Convert.ToInt32(Member);
                if (countMember <= 0)
                {
                    MessageBox.Show("Family's number of member can not less than 0, Please try again", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
            ...
            {
                MessageBox.Show("Last month's water number is a number, Please try again", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            string currentMonth = txtCurrentMonth.Text.Trim();
            if (!int.TryParse(currentMonth, out _))
            {
                MessageBox.Show("Curren month's month's water number is a number, Please try again", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            ...
            {
                MessageBox.Show("Current month's water can not be less than last month's water", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult dl = MessageBox.Show("Are you sure you want to delete this record?", "Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            if (lstVBill.SelectedItems.Count > 0)
            {
                ...
                if (string.IsNullOrEmpty(customer))
                {
                    MessageBox.Show("Enter your fullname, please try again", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                string customerType = cobCustomerType.Text.Trim().ToLower();
                if (string.IsNullOrEmpty(customerType))
                {
                    MessageBox.Show("Select Customer's type, please try again", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                int countMember = 0;
                if (customerType.Equals("household customer"))
                {
                    ...
                    if (!int.TryParse(member, out _))
                    {
                        MessageBox.Show("Enter number of familly's member, please try again", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                    countMember = Convert.ToInt32(member);
                    if (countMember <= 0)
                    {
                        MessageBox.Show("The number of familly's member, can't less than 0, please try again", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                }
                string lastMonth = txtLastMonthWater.Text.Trim();
                ...
                if (!int.TryParse(currentMonth, out _))
                {
                    MessageBox.Show("Current month's water number is a number, please try again", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                int lastMonthWater = Convert.ToInt32(lastMonth);
                ...
                {
                    MessageBox.Show("Current month's water can't be less than last month's water", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
